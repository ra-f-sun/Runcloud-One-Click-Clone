...
class OCC_Admin {
	...
	public function enqueue_assets( $hook ) {
		if ( 'toplevel_page_one-click-clone'!== $hook ) {
			return;
		}
	...
	public function display_main_page() {
		// If token exists, allow testing or show clone UI. For Step 1, we stick to Settings.
		$token_exists = get_option( 'occ_rc_api_token' );
		
		// Simple Router
		$view = isset( $_GET['view'] )? sanitize_text_field( $_GET['view'] ) : 'settings';
		
		// Force settings if no token
		if (! $token_exists ) {
			$view = 'settings';
		}
		...
		if ( $view === 'settings' ) {
			...
			echo '<div class="card"><h3>Clone Interface Coming in Step 2</h3></div>';
		}
		echo '</div>';
	}
